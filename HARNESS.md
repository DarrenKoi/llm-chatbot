...
| 이전 경로 | 현재 경로 |
|---|---|
| `api.mcp` / `api/mcp/` | `api.mcp_client` / `api/mcp_client/` |
| `devtools.mcp` / `devtools/mcp/` | `devtools.mcp_client` / `devtools/mcp_client/` |
| `api.mcp_runtime` / `api/mcp_runtime/` | `api.mcp_client` / `api/mcp_client/` |
| `devtools.mcp_runtime` / `devtools/mcp_runtime/` | `devtools.mcp_client` / `devtools/mcp_client/` |

- 호환성 shim(`api/mcp/__init__.py`)이 제거되었으므로 `from api.mcp import ...` 형태의 import는 즉시 ImportError가 납니다. 직전 단계 이름이었던 `from api.mcp_runtime import ...` 도 더 이상 동작하지 않으므로 새 경로로 갱신해 주십시오.
...
2. 작업 범위는 기본적으로 `api/mcp_client/`, `api/workflows/`, `devtools/workflows/`에 한정한다.
...
작업 범위는 기본적으로 api/workflows/, api/workflows/start_chat/, api/mcp_client/, devtools/workflows/로 제한한다.
...
api/mcp/, devtools/mcp/ 와 api/mcp_runtime/, devtools/mcp_runtime/ 경로는 더 이상 존재하지 않는다. 코드나 import에서 이 경로가 등장하면 즉시 멈추고 보고해라. 새 경로는 api/mcp_client/, devtools/mcp_client/ 이다.
...
- `dev_orchestrator.py`의 운영 `api.workflows.start_chat` cross-import는 `devtools/workflows/start_chat/` 전용 그래프로 대체되어 제거됐습니다. 워크플로 런타임 경로의 cross-import는 현재 0건이며, 남은 예외는 `devtools/cube_message/samples.py`(수동 Cube 페이로드 점검 도구, 격리 클린업 의도적 면제)뿐입니다.
...
### devtools start_chat 전용 그래프

dev runner에서 "신규 핸드오프 워크플로 promote 전 라우팅 검증"을 수행할 수 있도록 `devtools/workflows/start_chat/`에 별도 entry 그래프가 존재합니다. 운영 `api/workflows/start_chat/`과는 의도적으로 분리됩니다.

- **목적:** classify 노드가 `devtools/workflows/`의 `handoff_keywords`를 매칭해 sub-workflow로 라우팅되는지를 promote 전에 확인하는 것. 신규 워크플로를 dev에서 작성→ 라우팅 확인 → promote라는 흐름을 만들어 줍니다.
- **노드 구성:** `entry`(no-op), `classify`(devtools 레지스트리 키워드 매칭), `noop_reply`(매칭 실패 시 자리표시). 운영 start_chat의 `retrieve_context_node`, `generate_reply_node`, RAG/file/profile/LLM 호출은 미러링하지 않습니다 — dev의 목적은 응답 품질이 아니라 라우팅 검증입니다.
- **상태:** `DevStartChatState`는 `ChatState`에 `active_workflow`, `handoff_match_reason`만 더한 가벼운 사본입니다. 운영 `StartChatState`의 `profile_*`, `retrieved_contexts`는 포함하지 않습니다.
- **promote 책임 범위에서 제외:** `devtools/workflows/start_chat/`은 dev 전용 영구 인프라이며 promote 대상이 아닙니다. `devtools/scripts/promote.py`는 `workflow_id == "start_chat"`을 명시적으로 거부합니다.
- **mirror 동기화 책임:** 운영 start_chat의 classify 시그니처/엣지 패턴이 바뀌면 dev 사본도 같은 PR에서 함께 업데이트합니다(위 mirror 규칙 적용). 단, RAG/LLM 노드 변경은 dev 사본 동기화 의무가 없습니다.
...
1. 수정 파일이 정말로 `api/workflows/`, `api/mcp_client/`, `devtools/workflows/` 중심인가?
...
- 관련 없는 통합을 건드리기 전에 `api/mcp_client/`에서 먼저 수정합니다.
