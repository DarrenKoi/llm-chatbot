...
현재 구조에서는 키워드를 아래 절차로 정하는 편이 가장 안전합니다.

1. 사용자가 실제로 말할 업무 요청 문장을 먼저 수집합니다.
2. 그 문장에서 공통으로 반복되는 표현만 키워드 후보로 남깁니다.
3. 다른 워크플로와 겹치는 일반 명사는 제거합니다.
4. 한국어와 영어 요청이 모두 들어오는 업무면 두 언어 표현을 같이 둡니다.
5. `start_chat`에 직접 넣어 양성 예시와 음성 예시를 모두 확인합니다.

즉, 키워드는 개발자가 떠오르는 단어를 넓게 넣는 방식보다, 실제 사용자 발화에서 좁고 분명한 표현을 뽑아내는 방식이 더 잘 맞습니다.

### 좋은 키워드와 나쁜 키워드의 차이

- 좋은 키워드는 업무 목적이 바로 드러나는 표현입니다.
- 좋은 키워드는 다른 워크플로 키워드와 겹치지 않는 표현입니다.
- 좋은 키워드는 실제 사용자 문장에 그대로 등장할 확률이 높은 표현입니다.
- 나쁜 키워드는 `도움`, `계획`, `문서`, `분석`처럼 범위가 넓은 표현입니다.
- 나쁜 키워드는 한 워크플로만의 의미가 없는 상위 개념 표현입니다.

예를 들어 여행 워크플로라면 `여행 계획`, `여행 일정`, `travel planner`는 괜찮지만 `계획`만 단독으로 두는 방식은 좋지 않습니다.

### 현재 구현에서 겹치는 키워드가 위험한 이유

현재 구현은 사용자 메시지를 소문자로 바꾼 뒤 `handoff_keywords`를 앞에서부터 순서대로 검사합니다.

워크플로 발견 순서는 패키지 스캔 순서의 영향을 받으므로, 같은 단어를 여러 워크플로가 공유하면 먼저 발견된 워크플로가 매칭될 수 있습니다.

즉, 키워드 중복은 단순 품질 문제가 아니라 실제 오라우팅 문제로 이어질 수 있습니다.

### 추천 검증 방식

키워드를 등록한 뒤에는 아래 두 묶음을 반드시 같이 점검하는 편이 좋습니다.

- 양성 문장은 이 워크플로로 들어가야 하는 문장입니다.
- 음성 문장은 비슷해 보여도 다른 워크플로로 가거나 일반 대화로 남아야 하는 문장입니다.

예를 들어 번역 워크플로라면 아래처럼 검증할 수 있습니다.

- 양성 문장은 `이 문장을 영어로 번역해 줘`, `translate this to Korean` 같은 문장입니다.
- 음성 문장은 `영어 공부 자료 추천해 줘`, `일본 여행 일정 만들어 줘` 같은 문장입니다.

이 검증은 dev runner에서 `start_chat`을 선택하고 직접 문장을 넣어 확인하는 방식이 가장 빠릅니다.
