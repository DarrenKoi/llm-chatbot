...
운영 반영 시 같은 이름으로 `api/mcp_client/` 아래로 이동하므로 파일명과 역할을 일치시켜 두는 편이 좋습니다.
...
`promote` 스크립트는 `devtools.mcp_client.` → `api.mcp_client.` 경로만 치환하므로, 나머지 import는 처음부터 운영 경로 기준으로 작성해야 합니다.

### MCP import

dev 단계에서는 템플릿처럼 `devtools.mcp_client.<workflow_id>`를 사용합니다.

`promote` 스크립트가 운영 반영 시 이 경로를 `api.mcp_client.<workflow_id>`로 자동 치환합니다.
...
2. `devtools/mcp_client/sample_flow.py`를 `api/mcp_client/sample_flow.py`로 복사합니다.
3. `devtools.mcp_client.` import를 `api.mcp_client.` import로 자동 치환합니다.
4. import 검증을 수행합니다.
