...
6. 특정 업무 의도가 감지되면 `translator`, `travel_planner`, `chart_maker` 같은 서브그래프로 분기한다.
...
- 서브그래프 예시:
  ...
  - `api/workflows/chart_maker/lg_graph.py`
...
### 4.4 `chart_maker`: 아직 스켈레톤 성격이 강함

`chart_maker`는 현재 구조 예시로는 유용하지만, 실제 업무 로직은 매우 얇다.
...
즉, “LangGraph를 어떻게 붙이는가”는 보여주지만, production-grade 업무 그래프라고 보기는 어렵다.
...
### 8.4 `chart_maker`는 production workflow라기보다 예제 성격이 강하다

현재 `chart_maker`는 구조 데모로는 유용하지만, 실제 차트 생성 업무를 담당하는 수준은 아니다.  
향후 실제 기능으로 키울 계획이라면 다음을 먼저 정해야 한다.
