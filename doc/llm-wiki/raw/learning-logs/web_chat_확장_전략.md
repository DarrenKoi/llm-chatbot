...
이번 확장안의 전제는 아래와 같습니다.

- 프론트엔드는 Nuxt를 사용합니다.
- Nuxt는 `ssr: false` SPA 모드로 운영합니다.
- UI는 Nuxt UI를 기준으로 구성합니다.
- 사내 기본 SSO 프로그램이 사용자를 인증하고, Flask는 SSO가 내려준 `LASTUSER` cookie로 현재 사용자를 식별합니다.
- `LASTUSER`는 이 저장소에서 사용할 `unique_name` 또는 `user_id`와 같은 값으로 취급합니다.
...
1. Flask가 `LASTUSER` cookie에서 현재 사용자를 식별합니다.
2. 사용자 메시지를 conversation store에 저장합니다.
3. 같은 `user_id`와 `channel_id`로 LangGraph workflow를 호출합니다.
4. assistant 응답을 conversation store에 저장합니다.
5. 응답을 Cube가 아니라 웹 API JSON으로 반환합니다.
...
Nuxt는 Flask와 별도로 개발하되, 운영에서는 `ssr: false`로 빌드한 정적 SPA 파일을 Flask가 제공하는 방식을 권장합니다. 이렇게 하면 운영 서버에서 Node SSR 프로세스를 따로 띄우지 않아도 됩니다.
...
- `identity.py`: `request.cookies["LASTUSER"]` 기반으로 현재 사용자의 `user_id`, `user_name`을 결정합니다.
...
`nuxt.config.ts`는 아래 방향을 기본으로 둡니다.
...
운영에서는 Nuxt build 결과물을 `api/static/chat/` 또는 배포 artifact 경로로 복사합니다. 개발 중에는 Nuxt dev server에서 Flask API를 호출하되, local SSO cookie가 없는 환경에서는 테스트용 cookie 주입 또는 dev-only 인증 우회를 별도로 정해야 합니다.

## 5. 사용자 식별 전략

사내 기본 SSO 프로그램이 인증을 담당하므로 이 저장소는 자체 로그인 화면을 만들지 않습니다. Flask API는 매 요청마다 `LASTUSER` cookie를 읽어 현재 사용자를 결정합니다.

```text
LASTUSER cookie value = unique_name = user_id
```
...
def get_current_web_chat_user() -> WebChatUser:
    raw_user = request.cookies.get("LASTUSER", "").strip()
    if not raw_user:
        raise Unauthorized("LASTUSER cookie is required.")

    return WebChatUser(user_id=raw_user, user_name=raw_user)
```

보안상 중요한 점은 아래와 같습니다.

- 브라우저 요청 body에 들어온 `user_id`는 사용하지 않습니다.
- Nuxt가 `LASTUSER`를 직접 읽어서 인증 판단을 하지 않습니다.
- Flask가 서버 측에서 `LASTUSER`를 읽고, 필요한 경우 `/api/v1/web-chat/me`로 현재 사용자 표시 정보만 내려줍니다.
- `LASTUSER`를 신뢰하려면 Flask가 반드시 사내 SSO 또는 허용된 reverse proxy 뒤에서만 노출되어야 합니다.
- SSO 또는 proxy가 `LASTUSER`를 항상 설정하고 외부 사용자가 임의 cookie로 우회할 수 없도록 운영 경계를 확인해야 합니다.
- `LASTUSER`가 없으면 401 응답을 반환하거나 사내 SSO 진입 경로로 redirect하는 정책을 정합니다.
...
브라우저가 `user_id`를 직접 보내게 만들지 않는 것이 중요합니다. `user_id`는 항상 Flask의 `identity.py`가 `LASTUSER` cookie에서 결정해야 합니다.
...
```text
user_id = LASTUSER cookie 값
channel_id = Cube에서 사용하던 채널 ID 또는 웹 전용 conversation ID
```
...
- 중앙: 메시지 timeline
- 하단: 입력 composer
- 상단 header: 현재 대화 ID, 연결 상태, 새 대화 버튼, 현재 사용자 표시
...
- `ConversationHeader.vue`: 현재 사용자, 현재 대화 정보, 연결 상태 표시
- `useChatApi.ts`: Flask API 호출을 캡슐화

Nuxt UI는 화면 구성과 상호작용 컴포넌트의 기본값으로 사용합니다. 별도 chat framework나 prototype 도구를 검토하지 않고, 1차 구현은 Nuxt UI와 직접 작성한 Flask JSON API 연동으로 제한합니다.

## 10. Nuxt API 호출 정책

Nuxt는 `ssr: false`이므로 사용자 식별과 권한 검증을 Nuxt server route에 맡기지 않습니다. 모든 인증 판단은 Flask API에서 처리합니다.

`useChatApi.ts`의 기본 방향은 아래와 같습니다.

```ts
const apiBase = useRuntimeConfig().public.webChatApiBase
...
export async function sendMessage(conversationId: string, message: string) {
  return await $fetch(`${apiBase}/conversations/${conversationId}/messages`, {
    method: 'POST',
    credentials: 'include',
    body: { message },
  })
...
운영에서는 `/chat` 화면과 `/api/v1/web-chat` API가 같은 Flask origin에서 제공되므로 cookie 전달이 단순합니다. 개발 중 Nuxt dev server와 Flask가 다른 origin이면 `credentials: 'include'`, CORS credentials 허용, cookie domain 정책을 함께 맞춰야 합니다.
...
- 현재 사내 SSO cookie 기반 운영 방식과 잘 맞습니다.

### 12.2 대안: Nuxt SSR 별도 운영

현재 조건에서는 권장하지 않습니다. Nuxt SSR이 꼭 필요하면 Flask와 Nuxt를 별도 서비스로 운영할 수 있지만, reverse proxy, CORS, session 공유, SSO cookie 전달, 배포 단위가 늘어납니다.

현재 목적은 Cube 장애 대비 fallback 채널이므로 1차 구현에서는 `ssr: false` SPA가 더 적합합니다.

## 13. 단계별 도입 계획

### 13.1 웹 채팅 backend API 추가

- `api/web_chat/identity.py` 추가
- `LASTUSER` cookie 기반 `WebChatUser` 생성
...
- pytest로 인증, 저장, workflow 호출, 오류 응답을 검증

### 13.2 conversation 목록 API 추가
...
- `LASTUSER` 기준으로 본인 conversation만 반환

### 13.3 Nuxt chat UI 추가

- `web/` 디렉터리에 Nuxt app 생성
- `nuxt.config.ts`에 `ssr: false`, `@nuxt/ui`, `/chat/` baseURL 설정
- `/chat` 첫 화면을 실제 채팅 UI로 구성
- 메시지 전송, 응답 표시, loading, error 상태 구현
- Flask API와 같은 origin 기준으로 연동

### 13.4 Flask static serving 추가

- Nuxt build 결과물을 `api/static/chat/` 또는 별도 artifact 위치에 배치
- `/chat`과 `/chat/<path>` route 추가
- asset cache header와 404 fallback 정책 정리
- `index.html` fallback이 API route를 가로채지 않도록 route prefix를 분리

### 13.5 운영 검증

- SSO 경유 요청에서 `LASTUSER` cookie가 Flask까지 전달되는지 확인
- `LASTUSER` 값이 Cube의 사용자 ID 또는 `unique_name`과 일치하는지 확인
...
- `LASTUSER` cookie가 없으면 `/api/v1/web-chat/me`와 message API가 401을 반환하는지 검증합니다.
- `LASTUSER` cookie가 있으면 `user_id`가 해당 cookie 값으로 결정되는지 검증합니다.
- 요청 body에 `user_id`가 있어도 서버가 무시하는지 검증합니다.
...
- `LASTUSER` cookie는 Flask 서버가 읽는 인증 입력입니다. Nuxt 화면에서 임의로 만든 사용자 값으로 대체하면 안 됩니다.
- `LASTUSER`를 신뢰할 수 있는지는 애플리케이션 코드만으로 보장되지 않습니다. 사내 SSO 또는 proxy가 cookie를 설정하고 외부 직접 접근을 막는 운영 구성이 함께 필요합니다.
...
2. Nuxt는 `web/` 아래에서 개발하고 `ssr: false` SPA로 빌드합니다.
3. Nuxt UI를 기본 UI 시스템으로 사용합니다.
4. Nuxt build 결과만 Flask가 `/chat`에서 제공합니다.
5. 웹 채팅 API는 `api/web_chat/`에 별도 모듈로 둡니다.
6. Flask는 매 요청마다 `LASTUSER` cookie를 읽어 `user_id`로 사용합니다.
7. workflow 호출은 `api.workflows.lg_orchestrator.handle_message()`를 공유합니다.
8. Cube 전송 함수는 웹 채팅 경로에서 호출하지 않습니다.
9. 같은 대화를 이어가야 할 때는 반드시 같은 `user_id`와 같은 `conversation_id/channel_id`를 사용합니다.
10. 장기적으로 `CubeIncomingMessage`를 공통 `IncomingMessage` 모델로 분리합니다.
