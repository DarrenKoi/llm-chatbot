...
| `text_summary.json` | 텍스트 요약 카드 샘플. |
| `grid_table.json` | 그리드 테이블 샘플. |
| `select_callback.json` | select 콜백 샘플. `callbackaddress`가 비어 있으면 config 값으로 채워진다. |
| `extensionless_sample` | 확장자가 없어도 로드되는지 확인하는 샘플. |
...
같은 디렉터리에 자신의 `richnotification` JSON 파일을 추가하고 새 함수에서 `send_raw_file()`을 호출한다.
...
| `list_richnotification_files()` | 디렉터리에 있는 사용 가능한 샘플 파일 경로 목록. |
