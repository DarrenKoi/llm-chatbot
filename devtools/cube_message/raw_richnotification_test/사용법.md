...
| `config.py` | 자격증명·전송 대상 ID·기본 콜백 주소 등의 로컬 설정. **`.gitignore`로 추적 제외**됨. |
| `config.py.example` | `config.py`의 템플릿. 처음 사용 시 복사해서 본인 값으로 채운다. |
| `samples/` | raw richnotification JSON 페이로드를 모아 두는 폴더. 새 페이로드를 추가하려면 이 폴더에 파일을 떨어뜨리면 된다. |

## 빠른 사용법

1. **`config.py`를 만들고 채운다.** 처음 한 번만 템플릿을 복사한다.

   ```bash
   cp config.py.example config.py
   ```

   그 다음 `config.py`를 열어 봇 ID/토큰과 본인 Cube `uniquename`을 채운다. 비어 있는 값은 프로젝트 루트의 `.env`에서 자동 보충되며 (`CubeMessageConfig.from_env()`), richnotification URL은 `client.py`의 `DEFAULT_CUBE_API_URL`에서 자동으로 가져온다.
   ...
   `config.py`는 `.gitignore`에 등록되어 있으므로 실제 토큰을 넣어도 커밋되지 않는다. 새로운 설정 항목을 팀에 공유하려면 `config.py.example`에만 추가한다.
