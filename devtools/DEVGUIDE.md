...
- `start_chat` 워크플로는 운영 그래프를 그대로 import하므로 노드 안에서 `api.conversation_service.get_history`가 호출됩니다. dev runner는 시작 시 `CONVERSATION_BACKEND=memory`를 강제로 주입해 `.env`에 `AFM_MONGO_URI`가 남아있어도 도달 불가능한 MongoDB에 연결을 시도하지 않습니다 (`devtools/workflow_runner/app.py`).
