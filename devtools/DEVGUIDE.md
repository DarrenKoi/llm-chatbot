...
#    - devtools/workflows/travel_planner/
#    - devtools/mcp_client/<workflow_id>.py

# 3. dev runner 실행
python -m devtools.workflow_runner.app

# 4. 브라우저에서 http://localhost:5001 접속 → 본인 workflow 선택 → 테스트
```
...
### 5. dev runner에서 workflow 선택 기준

`start_chat`은 사용자의 메시지가 등록된 workflow로 진입하는지 확인하는 라우팅 검증용 진입점입니다.
예를 들어 `handoff_keywords`가 `"여행"`인 workflow가 있으면 `start_chat`에서 "여행 계획 짜줘" 같은 메시지가 해당 workflow로 들어가는지 확인할 수 있습니다.

본인 workflow의 노드, interrupt/resume, state를 직접 테스트하려면 드롭다운에서 `start_chat`이 아니라 해당 workflow ID를 직접 선택하세요. `start_chat`에서 "해당 메시지로 원하시는 workflow로 들어가지 못했습니다."가 나오면 메시지가 어떤 등록 workflow의 `handoff_keywords`에도 매칭되지 않았다는 뜻입니다.

---

## 추천 작성 순서

1. 기본 구조는 `_template`, 멀티턴 흐름은 `travel_planner` 중 가까운 예제를 고릅니다. (Cube 페이로드 외형 점검은 워크플로가 아니라 `devtools/cube_message/` 도구로 합니다.)
...
5. dev runner 드롭다운에서 본인 workflow를 직접 선택해 노드 흐름을 먼저 검증합니다.
6. `start_chat`을 선택해 등록된 handoff workflow로 진입하는지도 검증합니다.
7. 실제 운영 연결은 `api/workflows/start_chat/`에서 handoff 기준으로 붙입니다.
...
- `start_chat` 워크플로는 dev 전용 라우팅 검증 그래프입니다. 운영 그래프를 그대로 import하지 않고 등록된 dev workflow의 `handoff_keywords` 매칭 여부만 확인합니다.

### LLM이 필요한 워크플로

- LLM을 호출하는 노드가 있다면 `.env`에 `LLM_BASE_URL`과 `LLM_API_KEY`가 설정되어 있어야 합니다.
- 규칙 기반 devtools 워크플로(`travel_planner` 등)는 LLM 없이 동작합니다.
